...
const baseUrl = process.env.REACT_APP_ATLAS_BASE_URL;
...
export function AppProvider({ appId, children }) {
  ...
  const removeDeleteUser = React.useCallback(async () => {
    try {
      console.log('Arrivederci Roma!');
      const user = app.currentUser;
  ...
  const appContext = React.useMemo(() => {
    return { ...app, currentUser, logIn, logOut, removeDeleteUser };
  }, [app, currentUser, logIn, logOut, removeDeleteUser]);
